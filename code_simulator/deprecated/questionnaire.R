...
# IT 127-grid: gpercentile vector + per-distribution value vector
...
# PI for World: cash_income_2025 × country-specific growth_pi, aggregated with PI pop (Z0b).
pi_pool       <- ci25
pi_pool$v_pi  <- pi_pool$cash_income_2025 * growth_pi_2025_2100[pi_pool$country]
pi_World_dist <- build_world_dist(pi_pool, "v_pi", "gpercentile", pop_pi_2100) * (1 - extra_tax_pi["World"])
# Income 2100 World distribution (Bothe SC) → SC pop (Z0a) at 2100
inc_World_2100 <- build_world_dist(inc_wide, "income_2100", "gpercentile", pop_sc_2100)

# IT and World PI level distributions
pi_IT_dist     <- ci25_IT * growth_pi_2025_2100["IT"] * (1 - extra_tax_pi["IT"])
# SC2 = income_2100 × (avg_cash_2025 / avg_income_2025), then multiplied by
# (1 − extra_tax_sc[X]) to apply the SC extra educ/health/PS tax as a flat
# proportional tax (same percentage at every gpercentile, denominator is the
# pre-tax SC-scenario avg_cash_income at 2100).
sc2_IT    <- inc_IT_2100    * (avg_cash[["2025"]]["IT"]    / avg_inc[["2025"]]["IT"])    * (1 - extra_tax_sc["IT"])
sc2_World <- inc_World_2100 * (avg_cash[["2025"]]["World"] / avg_inc[["2025"]]["World"]) * (1 - extra_tax_sc["World"])
