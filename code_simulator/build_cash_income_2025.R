...
##### 1. FG 2023 (slim if available, full otherwise) #####
...
##### 3. Per-country dimensionless components #####
...
##### 6. Convert gperc index → Bothe lower-bound, export #####
...
write.csv(out, "../distributions/cash_income_2025.csv", row.names = FALSE, quote = FALSE)
message(sprintf("Wrote distributions/cash_income_2025.csv: %d rows (%d countries)",
...
write.csv(out_imp, "../distributions/imputed_income_2025.csv", row.names = FALSE, quote = FALSE)
message(sprintf("Wrote distributions/imputed_income_2025.csv: %d rows", nrow(out_imp)))
...
post_GIT_path <- if (file.exists("../distributions/income.csv")) "../distributions/income.csv" else "../distributions/income_full_revenues.csv"
...
write.csv(ts_out, "../distributions/cash_income.csv", row.names = FALSE, quote = FALSE)
message(sprintf("Wrote distributions/cash_income.csv: %d rows × %d cols", nrow(ts_out), ncol(ts_out)))
...
write.csv(world_df, "../distributions/cash_income_world.csv", row.names = FALSE, quote = FALSE)
message(sprintf("Wrote distributions/cash_income_world.csv: 100 rows × %d cols", ncol(world_df)))
