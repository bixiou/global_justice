...
\begin{description}
  ...
  \item[\file{conjoint\_any2}.] Similar to \file{conjoint\_any} except that hourly productivity grows at Italy's  2025--2035 productivity rate modelled by \cite{Chancel2026} (close to 3\%), instead of the flat 1.5\,\%/year assumption used by \file{conjoint\_few} and \file{conjoint\_any}  (Section~\ref{sec:variants}).
\end{description}
...
\begin{description}
  \item[An hours factor.] Longer hours mean more output: relative to the 36\,h benchmark, the model moves income to the class's productivity level. The 40\,h ``\scenario{B90k}'' class is the reference of this family; its factor corrects the benchmark for the fact that the convergence scenario front-loads productivity growth and shortens the working week, so working a constant 40\,h can actually leave 2035 income a little \emph{below} the benchmark (Section~\ref{sec:bfamily}). The 32\,h and 44\,h classes are the 40\,h class scaled by their  worked-hours ratio (32/40, 44/40); the 36\,h ``\scenario{B}'' class uses the convergence scenario's own modelled change in hours per worker. % TODO
...
\noindent The benchmark \scenario{SC} is simply \col{IT35} with the fast-decarbonisation and public-services factors applied. \scenario{B90kC}, \scenario{B45kC}, \scenario{B}, \scenario{PC} and so on add the corresponding hours factor; the scope variants substitute the appropriate base and scope adjustment. The \scenario{PI} (long-hours, no-redistribution) scenario is the only one built directly from 2025 cash income rather than from \col{IT35}, because it never undergoes the convergence transition --- it simply grows the current distribution by a flat GDP factor of 1.4. % TODO
...
On top of the base temperature, two additive corrections capture the food and travel choices --- $-0.24$\,°C for cutting beef and $-0.155$\,°C for cutting flights --- and a GDP-dependent correction captures the emissions effect of expanded public services. Which corrections
apply depends on the decarbonisation pace: under slow and intermediate decarbonisation the canonical case is fixed by the public-services choice and beef/flights are adjustments; under fast decarbonisation, where land use dominates, the canonical case is fixed by the beef choice and flights/public services are the adjustments. % TODO clarify
...
\paragraph{The respondent's own 2035 income.} It converts the household's monthly cash income to an annual individual income (halving it for couples), locates that income's percentile in the 2025 cash distribution by interpolation, and then reads the chosen scenario's 2035 column at that percentile. The chosen column is assembled on the fly from the exported base columns by applying the same hours, scope, decarbonisation and public-services factors described in Section~\ref{sec:factors}, exactly reproducing the precomputed values. 
(The percentile search uses only the bottom 99 brackets, where the cash distribution is monotone; above the 99th percentile the top bracket's income is returned.) % TODO!
...
\begin{description}
  \item[\file{ineq\_IT\_2035.csv} (and \file{\_full}).] 127 g-percentiles $\times$ the 2035 cash-income scenario columns, the 2025 reference and the net global transfer \col{cash\_GR35}. The \file{\_full} version carries every hours-class $\times$ scope combination; the trimmed survey copy keeps only the columns the engine reads.
  \item[\file{ineq2\_IT\_2035.csv} (and \file{\_full}).] The same, with the B scenarios using each country's own 2025--2035 productivity growth (Section~\ref{sec:variants}).
