...
save_table(results_h1h4, "results_h1h4",
...
save_table(results_het_primary, "results_heterogeneity_primary", group_by = "Demographic",
           caption = "Heterogeneity: primary specification, AMCEs interacted with respondent characteristics (one demographic at a time).",
           notes = "Baselines: Age vs. 18-34; Gender vs. Female (\"Other\", N=4, excluded); Region vs. Northwest; Education vs. Compulsory/lower secondary. Cluster-robust (respondent-level) SE in parentheses. *** p$<$0.01, ** p$<$0.05, * p$<$0.1 (unadjusted p-values - exploratory heterogeneity analysis, not part of the FDR-corrected pre-registered family). Political orientation not yet available (placeholder pending wave 1 merge).")
...
save_table(results_h1h4_robustness_income, "results_h1h4_robustness_income",
           caption = paste0("H1--H4 robustness check: primary specification re-estimated excluding respondents whose declared current income, annualized and adjusted for household size, exceeds the 99th percentile of the (2025) Italian income distribution (", n_participants_h1h4 - n_participants_robust, " respondent(s) excluded; threshold = ", ITALY_INCOME_P99_ANNUAL_PER_ADULT, " EUR/year per adult)."),
           notes = "Same specification, columns and stars convention as the primary H1--H4 table - compare directly to check sensitivity to this exclusion.")
...
save_table(results_secondary, "results_secondary_best_spec",
           caption = paste0("Secondary specification (highest adjusted R\\textsuperscript{2} among the pre-registered grid of ", nrow(grid_ranked), " alternatives varying income inclusion/functional form, temperature/hours functional form, and an extra growth$\\times$income interaction)."),
           notes = paste0("Cluster-robust (respondent-level) SE in parentheses. *** p$<$0.01, ** p$<$0.05, * p$<$0.1 (unadjusted p-values - this is a single selected descriptive model, not a pre-registered hypothesis-test family, so no FDR correction applies). Winning formula: ", gsub("_", "\\\\_", attr(results_secondary, "formula")), "."))
...
save_table(results_het_secondary, "results_heterogeneity_secondary", group_by = "Demographic",
...
save_table(grid_top15, "results_secondary_spec_search", digits = 4,
           caption = paste0("Secondary specification search: top 15 of ", nrow(grid_ranked), " candidate specifications by adjusted R\\textsuperscript{2}."),
           notes = "Q5 grid: income inclusion/functional form, temperature/hours functional form, extra growth$\\times$income interaction. Rank 1 is the winning specification reported in Table~\\ref{tab:results_secondary_best_spec}.")
...
save_table(results_h1h4_en, "results_h1h4_elasticnet",
           caption = paste0("Primary H1--H4 specification, re-estimated with ", length(en_terms), " interaction term(s) selected via Elastic Net added to the base specification."),
           notes = "$\\alpha$=0.5, main effects unpenalized, lambda.1se. Cluster-robust (respondent-level) SE in parentheses. *** p$<$0.01, ** p$<$0.05, * p$<$0.1 (unadjusted; descriptive, not part of the FDR-corrected pre-registered family). Compare to Table~\\ref{tab:results_h1h4} (without these interactions).")
...
save_table(results_secondary_en, "results_secondary_elasticnet",
           caption = paste0("Secondary (highest adjusted R\\textsuperscript{2}) H1--H4 specification, re-estimated with the same ", length(en_terms), " Elastic-Net-selected interaction term(s) added."),
           notes = "Cluster-robust (respondent-level) SE in parentheses. *** p$<$0.01, ** p$<$0.05, * p$<$0.1 (unadjusted; descriptive). Compare to Table~\\ref{tab:results_secondary_best_spec} (without these interactions).")
...
save_table(results_h5h11, "results_h5h11",
...
save_table(bt_table, "bt_ranking_table",
           ...
           notes = "Ability = log-strength relative to BC (reference, fixed at 0); SE in parentheses. *** p$<$0.01, ** p$<$0.05, * p$<$0.1 (FDR-adjusted Wald test vs. BC). R2 = McFadden pseudo-R2.")
...
save_table(results_idk_frequency, "results_idk_frequency",
           caption = "Frequency of ``I don't know'' responses by task, and for any of the 3 tasks.",
           notes = "SE in parentheses (normal approximation).")
...
save_table(results_idk_predictors, "results_idk_predictors",
           caption = "Predictors of ``I don't know'' responses: sociodemographic characteristics and task/scenario features.",
           notes = "Pooled linear probability model across the 3 DCE tasks, cluster-robust respondent-level SE in parentheses. *** p$<$0.01, ** p$<$0.05, * p$<$0.1. Baselines: Age vs. 18-34; Gender vs. Female (\"Other\", N=4, excluded); Region vs. Northwest; Education vs. Compulsory/lower secondary; Task vs. Task 2 (ANY). Political orientation not yet available (placeholder pending wave 1 merge).")
